-- Ölü/tehlikeli handle_invoice_stock overload'larını temizle.
-- Gerçek çağıran kodlar (n8n stok workflow, app.js, api/parse-invoice.js,
-- supabase/functions/parse-invoice-pdf) hepsi p_barcode/p_name/p_qty/p_type
-- isimli tek overload'a çağrı yapıyor; diğerleri hiç tetiklenmiyordu.
DROP FUNCTION IF EXISTS public.handle_invoice_stock(text, integer, integer);
DROP FUNCTION IF EXISTS public.handle_invoice_stock(text, numeric, integer);
DROP FUNCTION IF EXISTS public.handle_invoice_stock(text, integer, integer, text);

-- Gerçek çalışan overload'ı düzelt:
-- 1) Eşleşmeyen ürün artık skip değil, 0 stokla otomatik oluşturuluyor
-- 2) Satışta stok GREATEST(0,...) ile negatife düşmüyor
