-- handle_invoice_stock sadece products.stock'u güncelliyordu, stock_movements'a hiç
-- yazmıyordu. Audit trail sadece dashboard'daki client-side logMovement() çağrısından
-- geliyordu — n8n RPC'yi doğrudan çağırdığı için (Workflow 1/2/3, reversal RPC'leri de
-- dahil, hepsi handle_invoice_stock'a düşüyor) gerçek stok değişiklikleri Hareket
-- Geçmişi'nde hiç görünmüyordu. Artık fonksiyon kendi audit satırını kendi yazıyor;
-- auth.uid()/auth.email() ile hangi bağlamda çağrıldığı (n8n/service_role -> NULL,
-- dashboard authenticated -> gerçek kullanıcı) otomatik ayrılıyor.
...
BEGIN
  ...
  v_notes       := CASE WHEN auth.uid() IS NULL
                     THEN 'Mikro n8n otomasyonu (irsaliye/fatura) ile otomatik güncelleme'
