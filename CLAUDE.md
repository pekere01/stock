...
> Tarayıcıya gönderilen her kod herkese açıktır. Service key tüm RLS'i bypass eder; ele geçirilirse veritabanı tamamen açılır.
>
> **KURAL 2:** Auth.Admin işlemleri (kullanıcı oluştur / sil / güncelle) yalnızca **Supabase Edge Function** içinden yapılır.
> Edge Function JWT ile caller kimliğini doğrular, `user_permissions` tablosundan admin yetkisini kontrol eder, ardından service role ile işlemi gerçekleştirir.
>
> **KURAL 3:** Frontend sadece `sb` (anon/publishable key) ile çalışır. `sbAdmin` isimli değişken **hiçbir frontend dosyasında bulunmamalıdır.**
>
> Mevcut mimari: bkz. [[Güvenlik-Mimarisi]]
